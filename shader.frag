...
float sdBox(vec3 p, vec3 s) {
    ...
    p = 1.1 + .1 * cos(p  + p.x * 8. + u_time)-s;
    //p.xz += .5 + .5 * cos(4. * p.y + u_time);
...
vec2 uv = (gl_FragCoord.xy-1.0*u_resolution.xy)/u_resolution.y;
    // vec2 uv = gl_FragCoord.xy/u_resolution.y;
     ...
     vec3 ro = vec3(4.6 * cos(0.5 * u_time), cos(0.3 * u_time) * 2.5, 4.6 * sin(0.5 * u_time));
    ...
    if(d<MAX_DIST) {
        ...
        col.r += 0.15 + .3 * cos(10. * rf.y + 4. * u_time - 3.1415 / 2.);
        col.g += 0.15 + .3 * cos(10. * rf.y + 4. * u_time);
        col.b += 0.15 + .3 * cos(10. * rf.y + 4. * u_time + 3.1415 / 2.);
    } 
